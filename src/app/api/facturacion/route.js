...
// POST - Generar comprobante electronico (Boleta o Factura)
export async function POST(request) {
  try {
    ...
    if (!NUBEFACT_TOKEN) {
      ...
      return NextResponse.json({ error: 'Facturacion no configurada' }, { status: 500 });
    }
    ...
    // Generar numero de serie y correlativo
    // En produccion, esto deberia venir de la base de datos
    const serie = tipo_comprobante === 2 ? 'B001' : 'F001';

    // Preparar documento para Nubefact
    const documento = {
      operacion: 'generar_comprobante',
      tipo_de_comprobante: tipo_comprobante, // 1=Factura, 2=Boleta
      serie: serie,
      numero: venta_id, // Usar ID de venta como correlativo
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: resultado.errors || 'Error al generar comprobante',
        detalles: resultado
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      comprobante: {
        ...
        codigo_de_barras: resultado.codigo_de_barras
      }
    });
